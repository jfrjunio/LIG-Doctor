...
# \copy (select * from desnormalized order by patient_id, time) TO '/home/junio/Desktop/Ju/AGIR/AGIR-PREPROCESSED.csv' WITH (FORMAT CSV,HEADER TRUE)

global ARGS

def parse_arguments():
	parser = argparse.ArgumentParser()
	parser.add_argument('agir_file', type=str, default='AGIR-PREPROCESSED.csv', help='The AGIR dataset file.')
...
if __name__ == '__main__':
	...
	agir_ADMISSIONS_csv = open(ARGS.agir_file, 'r')
	# id, patient_id, action_id, time, gender, location, life, name -> name refers to action name, not patient name
	...
	for line in agir_ADMISSIONS_csv:
		...
		hadm_id = str(tokens[1]) + '-' + str(tokens[3])
	...
	for subject_id, hadmList in subjectTOhadms_Map.iteritems():
		...
		sortedList = sorted([(hadmTOadmttime_Map[hadm_id], hadmToactionIDCODEs_Map[hadm_id], hadm_id) for hadm_id in hadmList])
	...
	CODES_distributionMAP = entropy_analysis.writeDistributions(ARGS.agir_file, hadmToactionIDCODEs_Map, subjectTOhadms_Map, all_subjectsListOfCODEsList_LIST)
	for i, key in enumerate(CODES_distributionMAP):
		actionOrderedIndexesMAP[key[0]] = i
	entropy_analysis.computeShannonEntropyDistribution(all_subjectsListOfCODEsList_LIST, CODES_distributionMAP, ARGS.agir_file)
	...
	if (len(partitions) >= 1):
		...
		print 'Writing ' + str(partitions[0]) + '% of the patients read from file ' + ARGS.agir_file
		...
		if (len(partitions) >= 2):
			print 'Writing ' + str(partitions[1]) + '% of the patients read from file ' + ARGS.agir_file
			...
			if (len(partitions) >= 3):
				print 'Writing ' + str(partitions[2]) + '% of the patients read from file ' + ARGS.agir_file
				index_of_first_patient_to_dump = index_of_last_patient_to_dump
