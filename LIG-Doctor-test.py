...
def prepareHotVectorsWithFeatures(test_tensor, features_tensor):
...
def testModel():
	...
	xf, xb, featsSlice, mask, codePred = build_model()
	PREDICTOR_COMPILED = theano.function(inputs=[xf, xb, featsSlice, mask], outputs=codePred, name='PREDICTOR_COMPILED')
	...
	for batchIndex in range(nBatches):
		...
		xf, xb, featsSlice, mask, nVisitsOfEachPatient_List = prepareHotVectorsWithFeatures(batchX, batchT)
